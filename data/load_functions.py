...
def check_aska(name, gprotein_list, path):
	data = []
	for line in open(path+'/data/LogRAi_values_final.tsv', 'r'):
		if '#' in line:
			col_name = []
			col_name.append('GPCR')
			col_name += line.replace('\n', '').replace('\r', '').split('\t')[1:]
...
def extract_other_features(file, obj, base, atg):
	v = 0; w = 0; proteins = []
...
def check_iuphar(name, gprotein_list, d):
	...
	while len(l) <= 20:
		l += ' '
	dic = {'GNAI3':'Gi/Go', 'GNAI1':'Gi/Go', 'GNAO1':'Gi/Go', 'GNAZ':'Gi/Go', 'GNAL':'Gs', 'GNAS':'Gs', 'GNAQ':'Gq/G11', 'GNA14':'Gq/G11', 'GNA15':'Gq/G11', 'GNA12':'G12/G13', 'GNA13':'G12/G13'}
	for gprotein in gprotein_list:
...
def hmm_search(obj, path):
	l = ''
	for name in obj:
		l += '>' + str(name) + '\n'
		l += str(obj[name].seq) + '\n'
	open(path+'/temp/temp_fasta_file.txt', 'w').write(l)
	os.system('hmmsearch '+path+'/data/7tm_1.hmm '+path+'/temp/temp_fasta_file.txt > '+path+'/temp/temp_hmm_file.txt')
	return (path+'/temp/temp_hmm_file.txt')
